...
import { prepareEnvironmentInvitationTargets, prepareEnvironmentInvitationPlan } from '../src/invitation-environment.mjs';
...
export function parseArgs(argv) {
  ...
  if (!allowed[operation]) throw new TypeError('choose targets or plan');
  ...
  for (const key of ['environment','configuration','output', ...(operation === 'plan' ? ['targets','observations'] : [])]) {
  ...
  const inputs = ['environment','configuration','targets','observations','refinements'].filter(key => args[key]).map(key => path.resolve(args[key]));
...
export async function run(args, {createAccess} = {}) {
  ...
  const result = args.operation === 'targets'
    ...
    : await prepareEnvironmentInvitationPlan({access, configuration, targets:await readPrivateJson(args.targets),
...
export async function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    console.log(JSON.stringify({status:result.status ?? 'prepared', output:args.output,
